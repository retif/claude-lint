...
describe("skill-md linter", () => {
  it("passes for valid skill", () => {
    const diags = lintFile(resolve(FIXTURES, "valid-plugin/skills/example-skill/SKILL.md"));
    const errors = diags.filter((d) => d.severity === "error");
    expect(errors).toHaveLength(0);
  });

  it("reports missing frontmatter", () => {
    const diags = lintFile(resolve(FIXTURES, "invalid/skill-md/no-frontmatter.md"));
    expect(diags.some((d) => d.rule === "skill-md/valid-frontmatter")).toBe(true);
  });

  it("reports missing name", () => {
    const diags = lintFile(resolve(FIXTURES, "invalid/skill-md/missing-name.md"));
  ...
  it("reports missing trigger phrases", () => {
    const diags = lintFile(resolve(FIXTURES, "invalid/skill-md/no-triggers.md"));
    expect(diags.some((d) => d.rule === "skill-md/description-trigger-phrases")).toBe(true);
  });

  it("reports unknown frontmatter keys", () => {
    const diags = lintFile(resolve(FIXTURES, "invalid/skill-md/unknown-frontmatter.md"));
    const unknowns = diags.filter((d) => d.rule === "skill-md/no-unknown-frontmatter");
  ...
  it("reports description too long", () => {
    ...
    expect(diags.some((d) => d.rule === "skill-md/description-max-length")).toBe(true);
  });
  ...
  it("respects disabled rules", () => {
    ...
    const diags = skillMdLinter.lint("test.md",
      ...
      config);
    expect(diags.some((d) => d.rule === "skill-md/description-trigger-phrases")).toBe(false);
  });
});
